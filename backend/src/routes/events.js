...
router.get('/', async (req, res) => {
  try {
    const { type, upcoming, featured, limit = 100, page = 1 } = req.query
    ...
    query = query.order('date', { ascending: true }).range((page - 1) * limit, page * limit - 1)
...
router.get('/:id', async (req, res) => {
  try {
    const { data, error } = await supabase.from('website_events').select('*').eq('id', req.params.id).single()
